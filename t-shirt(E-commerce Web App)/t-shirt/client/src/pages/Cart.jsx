...
const Cart = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                // Fetch products one by one (as a fallback if API does not support multiple IDs)
                const fetchedProducts = await Promise.all(cartList.map(async (id) => {
                    const res = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/products/${id}`);
                    return res.data;
                }));
                if (isMounted) {
                    ...
                    <div className="xs:w-[95vw] sm:[90vw] md:w-5/6 mx-auto text-center sm:flex justify-between">
                        <div className="flex flex-col xs:text-xs md:text-sm p-1 sm:w-1/2 border-1">
                            {products.map((product) => {
                                ...
                                return (
                                    <div key={product._id} className='border-b-1 flex items-start relative xs:h-[110px] sm:h-[120px] py-4'>
                                        ...
                                        <div className='xs:w-2/3 flex flex-col justify-start items-start gap-2 text-sm font-medium'>
                                            <p className="xs:text-base md:text-lg text-left font-medium text-gray-900">{product.title}</p>
                                            <p className="xs:text-xs md:text-sm gap-2 font-medium text-gray-500">Qty:
                                                <button onClick={() => decCount(product._id, product.sellPrice)} className={`inline-flex ml-4 px-2 border-1 ${item.count <= 1 ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={item.count <= 1}>-</button>
                                                <p className='inline-flex px-2 border-1'>{item.count}</p>
                                                <button onClick={() => incCount(product._id, product.sellPrice)} className={`inline-flex px-2 border-1 ${item.count >= 5 ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={item.count >= 5}>+</button>
                                            </p>
                                            <div className='flex justify-between items-start flex-col w-full'>
                                                <p className="xs:text-xs md:text-sm text-gray-500">Price: {`\u20B9 ${new Intl.NumberFormat('en-IN').format(product.sellPrice)}`}</p>
                                                {item.subTotal !== product.sellPrice && (
                                                    <p className="xs:text-xs md:text-sm text-gray-500 justify-between">Subtotal: {`\u20B9 ${new Intl.NumberFormat('en-IN').format(item.subTotal)}`}</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
